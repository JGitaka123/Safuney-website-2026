...
export function CartLink({ className = "" }: { className?: string }) {
  ...
  return (
    <Link href="/cart" className={`relative inline-flex min-h-11 items-center gap-2 rounded-button px-3 text-body text-ink hover:bg-ground-deep ${className}`} aria-label={`Cart, ${count} ${count === 1 ? "item" : "items"}`}>
      ...
      {count > 0 ? <span className="tnum rounded-full bg-ink px-2 py-0.5 text-caption font-medium text-white">{count}</span> : null}
    </Link>
  );
}
