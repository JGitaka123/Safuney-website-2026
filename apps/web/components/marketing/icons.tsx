...
/*
 * Line icons for the storefront (ADR 0017), drawn on a 24 px grid with a 1.75 px stroke. Paths follow
 * Lucide (ISC licence); the WhatsApp glyph is the Simple Icons mark (CC0), used only on click-to-chat
 * links as WhatsApp's brand guidance allows. Icons are always decorative here — the text beside them
 * carries the meaning — so they are aria-hidden.
 */
const STROKED: Record<string, ReactNode> = {
...
export type IconName = keyof typeof STROKED | "whatsapp";

export function Icon({ name, className = "size-6" }: { name: IconName; className?: string }) {
  if (name === "whatsapp") {
    return (
      <svg aria-hidden focusable="false" viewBox="0 0 24 24" className={className} fill="currentColor">
        <path d={WHATSAPP} />
      </svg>
    );
  }
  return (
    <svg aria-hidden focusable="false" viewBox="0 0 24 24" className={className} fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
      {STROKED[name]}
    </svg>
  );
}
